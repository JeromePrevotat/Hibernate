...
public class AnnonceDao extends GenericDaoImp<Annonce, Long>{
    // String HQL_CHERCHER_PAR_TITRE = "FROM Annonce a WHERE a.titre LIKE :motcle";
    
    public AnnonceDao(SessionFactory sessionFactory){
        super(sessionFactory, Annonce.class);
    }
    

    // USING HQL
    // public List<Annonce> trouverParAuteur(long utilisateur_id){
    //     try(Session session = sessionFactory.openSession()){
    //         String hql = "FROM Annonce a WHERE a.auteur = :utilisateur_id";
    //         return session.createQuery(hql, Annonce.class)
    //                         .setParameter("utilisateur_id", utilisateur_id)
    //                         .getResultList();
    //     }
    // }

    // public List<Annonce> chercherParTitre(String motcle){
    //     try(Session session = sessionFactory.openSession()){
    ...
    public List<Annonce> criteriaSeach(Optional<String> titre, Optional<Long> auteur_id, String order, Optional<Integer> limit){
        try(Session session = sessionFactory.openSession()){
            Query<Annonce> query = null;
            CriteriaBuilder cb = session.getCriteriaBuilder();
            CriteriaQuery<Annonce> cq = cb.createQuery(Annonce.class);
            Root<Annonce> root = cq.from(Annonce.class);
